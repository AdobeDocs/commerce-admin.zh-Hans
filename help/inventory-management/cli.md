...
description: 了解用于管理清单数据和配置设置的 [!DNL Inventory Management Commerce] CLI命令。
...
- **升级至2.3.x，订单未处于最终状态（完成、已取消或已关闭）。** [!DNL Inventory Management]为这些订单创建补偿预留，但是它没有输入或从可销售数量中扣除的初始预留。 建议在从2.1.x或2.2.x升级到Adobe Commerce或Magento Open Source v2.3.x后使用这些命令。 如果您有待定订单，则这些命令会正确更新您的可销售数量和预留，以便进行销售和订单履行。
...
- **所有保留的总数不能解析为`0`。** 处于最终状态（“完成”、“已取消”、“已关闭”）的订单范围内的所有保留应解析为`0`，并清除所有可销售数量暂挂。
