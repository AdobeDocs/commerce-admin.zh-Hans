...
此 [《加州消费者隐私法案》][5] (CCPA)扩展了加州消费者决定如何收集、存储和使用其个人信息的权利。 其重点是保护消费者免受未经授权的出售或交换其个人信息之害。 CCPA于2018年颁布并于2020年1月1日生效。
...
CCPA还要求企业提供 _合理安全性_，并包括针对消费者的扩大数据保护条款，包括在发生数据泄露时采取法律行动的权利。
...
如果您的企业需要同时遵守两项 [通用数据保护条例](compliance-gdpr.md) (GDPR)和CCPA，您可以将来自GDPR合规计划的一些工作用于CCPA。 虽然这些法规有一些相似之处，但有一些不同之处，包括：
...
需要作出协调一致的努力，以制定和执行一项处理遵约问题的计划。 使用此路线图作为指南，来调动资源并安排任务的优先顺序，以便您能够在多个方面取得进展。 对于所有人来说，这个过程基本上是相同的 [!DNL Commerce] 安装，但以下情况除外：

- **云基础架构上的Adobe Commerce**：在Adobe上托管商店的商家 [云基础架构][4]{：target=&quot;_blank&quot;}可以咨询其Adobe Commerce技术客户经理或客户支持部门来获取有关响应消费者请求的帮助。
...
1. 确定通过网站和移动应用程序收集哪些公开和私人个人信息。 例如，标准Commerce数据库存储以下类型的公共和私人个人信息：
   ...
   - **私有**：客户信息、订单信息、奖励积分、礼品注册、通讯簿、商店信贷、支付方式、帐单协议、新闻稿订阅、邀请。
     ...
     如果您的 [!DNL Commerce] 已自定义安装，可能会收集其他个人信息。 个人信息可能还驻留在 [Cookie](./compliance-cookie-law.md)、标记和其他收集信息的技术。
...
1. 有关每个版本的适用数据流图和数据库实体映射，请参阅以下主题：
   ...
   ![图](./assets/privacy-frontend-diagram.svg)
...
- **CCPA范围内的企业实体** （品牌）： [!DNL Commerce] 商户收集并存储其顾客以及在店内购物的顾客的个人信息。
...
1. 建立响应过程并记录以下每个请求：
   ...
   - **选择退出的请求**  — 如果以有价值的对价出售或转让个人数据给第三方，CCPA要求 _不要出售我的信息_ 每个收集点的链接。 其他用户启用的输入控件（例如复选框和按钮）可用于电子邮件通信、网站首选项设置或网站表单中，以便个人提交有效的选择退出请求。
...
- **同意年龄**：如果您收集或使用未成年人的个人信息，可能需要遵守以下要求：
   ...
   - **13岁至16岁以下的未成年人**：至少13岁且不满16岁的未成年人可以选择出售其个人信息，前提是企业建立了合理的流程来记录此行为。 必须在公司的财务报表中说明此流程。 [隐私政策](privacy-policy.md). 当企业收到此年龄范围内的未成年人的请求时，必须告知他们以后可以选择退出的权利，并解释如何这样做。

  >[!IMPORTANT]
  >
  >商户不得将子女的个人资料储存在 [!DNL Commerce] 平台。 如果有理由相信收集的数据属于未成年人，则必须将其从 [!DNL Commerce] 平台的优惠，以免违反Adobe许可条款。
