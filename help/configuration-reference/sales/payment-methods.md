...
>[!TIP]
>
>适用于Adobe Commerce和Magento Open Source的Payment Services提供了一个可立即投入使用的自助服务解决方案，包括沙盒测试和简单的设置，用于提供强大而安全的支付处理。 要详细了解这个功能强大的工具集以及它如何为您提供所需的洞察信息和控制，以便为买家创造最佳体验，请参阅 [_Payment Services用户指南_](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/guide-overview.html).
...
><br/>
>如果已启用并配置其中任一捆绑扩展，则必须更新 `composer.json` 文件，以作为2.4.4升级过程的一部分，并管理以后的扩展更新。 请参阅 [升级模块](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/modules/upgrade.html) 在 _升级指南_ 以了解更多信息。<br/>
><br/>
...
- [PayPal Payflow Pro](paypal-payflow-pro.md) （包括“快速结帐”）
- [PayPal Payflow链接](paypal-payflow-link.md) （包括“快速结帐”）

## 基本支付方式

Commerce中内置以下支付方法，不使用第三方支付提供商处理交易。 许多基本支付方式是离线管理，而不是在线管理。
...
| [!UICONTROL New Order Status] | 网站 | 确定初始值 [订单状态](../../stores-purchase/order-status.md) 分配给支票或汇票支付的订单。 默认值： `Pending` |
...
>[!NOTE]
>
>具有的订单不支持分期付款 [多个配送地址](../../stores-purchase/shipping-settings.md#multiple-addresses) 和不会出现在付款选项中。
...
付款操作已配置 _每付款方式_. 付款活动确定获取资金的时间以及为销售订单创建发票的时间。
...
| [!UICONTROL Charge on shipment] | 在Commerce中创建发票时，Amazon会收到捕获请求并向客户收费。 |
| [!UICONTROL Charge on order] | Amazon将创建发票，并在下订单时向客户收费。 |
| [!UICONTROL Not Capture] | 提交发票时，系统不会捕获付款。 假定您稍后通过Commerce捕获付款。 完成的发票中有一个“捕获”按钮。 在捕获之前，您可以取消发票。 捕获后，您可以创建贷项通知单并撤消发票。 |
...
| [!UICONTROL New Order Status] | 网站 | 确定初始值 [订单状态](../../stores-purchase/order-status.md) 分配给按订单订单负责人支付的订单。 默认值：挂起 |
