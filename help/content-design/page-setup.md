...
![HTML页部分](./assets/storefront-home-html-inspect.png){width="700" zoomable="yes"}

## HTML头

HTML标题部分中的设置对应于 `<head>` HTML页面的标记，并且可以针对每个商店视图进行配置。 除了页面标题、描述和关键字的元数据之外，部分还包含指向网站图标的链接以及其他脚本。 本节中还配置了搜索引擎自动机的说明以及商店演示通知的显示。
...
1. 下 _其他设置_，展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL HTML Head]** 部分。

   ![HTML头配置设置](./assets/configuration-html-head.png){width="500" zoomable="yes"}

1. 更新 [法维孔](../getting-started/storefront-branding.md#add-a-favicon) 如果需要。
...
1. 输入任意 **[!UICONTROL Scripts and Style Sheets]** 根据需要。

1. 启用或禁用 [演示商店通知](../getting-started/storefront-branding.md#set-the-store-demo-notice) 如果需要。
...
| [!UICONTROL Scripts and Style Sheets] | 商店视图 | 包含必须在结束前包含在HTML中的脚本 `<head>` 标记之前。 例如，任何必须放在第三方网站之前 `<body>` 可以在此处输入标记。 |
...
![标题配置设置](./assets/configuration-header.png){width="400" zoomable="yes"}
...
1. 下 _其他设置_，展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL Header]** 部分。

1. 进行存储视图所需的任何更改：

   - [徽标](../getting-started/storefront-branding.md#upload-your-logo) 设置
   - [欢迎消息](../getting-started/storefront-branding.md#change-the-welcome-message) 设置
...
| [!UICONTROL Translate Title] | 商店视图 | 确定 `Page Title` 或 `Meta Title` 应该被翻译。 |

{style="table-layout:auto"}

## 页脚

在页脚配置部分，您可以更新 [版权声明](../getting-started/storefront-branding.md#change-the-copyright-notice) ，并输入必须在结束位置之前放置的其他脚本 `<body>` 标记之前。
...
1. 对进行必要的更改 **[!UICONTROL Copyright]** 和 **[!UICONTROL Miscellaneous HTML]** 设置。
...
| 字段 | 范围 | 描述 |
|--- |--- |--- |
| [!UICONTROL Miscellaneous HTML] | 商店视图 | 一个输入框，可以在其中将各种脚本上传到服务器，该脚本必须放在结束位置之前 `<body>` 标记之前。 |
