...
同步状态页面仅显示导出状态。 成功状态表示数据导出成功，并且最终可在连接的Commerce服务中使用。
...
## 验证数据是否已成功同步到Commerce服务

使用以下方法验证数据是否已成功与连接的Commerce服务同步：

* 对于云上或内部部署的Adobe Commerce，或者Adobe Commerce as a Cloud Service部署，请查看[数据管理功能板](data-dashboard.md)。
* 对于使用[Adobe Commerce Optimizer Connector](https://experienceleague.adobe.com/zh-hans/docs/commerce/aco-optimizer-connector/overview)配置的Adobe Commerce云上或本地部署，请在Commerce Optimizer Studio中检查[数据同步页面](https://experienceleague.adobe.com/zh-hans/docs/commerce/optimizer/setup/data-sync)。

>[!TIP]
>
>要了解有关数据同步过程的更多信息，请参阅&#x200B;*SaaS数据导出指南*&#x200B;中的[将数据与SaaS数据导出同步](https://experienceleague.adobe.com/zh-hans/docs/commerce/saas-data-export/data-synchronization)。
...
>[!NOTE]
>
>您不需要在[[!DNL Adobe Commerce as a Cloud Service]](https://experienceleague.adobe.com/zh-hans/docs/commerce/cloud-service/overview)实例上安装数据馈送状态扩展。
>如果在Commerce部署中至少启用了以下服务之一，则该扩展默认可用：产品推荐v6+、Live Search v4.1+或目录服务v1.17+。
...
您可以使用[!DNL Data Feed Sync Status - Details]页面上的[!DNL Actions]菜单手动重新同步失败或有问题的数据馈送。
...
>[!MORELIKETHIS]
>
>* [数据管理仪表板](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard)
