...
下表按默认显示顺序列出了典型产品导出的属性。 每个属性在CSV文件中都表示为一列，产品记录则表示为行。 以下划线开头的列包含服务数据，如复杂数据的属性或选项值。 您可以 [导出](data-export.md) 目录中的一个产品，用于查看数据中每个属性的表示方式。

用于导出此数据的安装已安装示例数据，并具有两个网站和多个商店视图。 尽管此列表包括通常导出的所有列，但 `sku` 是唯一需要的值。 要导入数据，只能包含更改后的列。 此 `sku` 应为第一列，但其余属性的顺序无关紧要。
...
| `product_type` | 指示产品类型。 值：<br/>`simple`  — 有形项目，通常作为单一件或固定数量出售。<br/>`grouped`  — 一组单独产品，作为一组产品销售。<br/>`configurable`  — 一种产品，具有客户在购买之前必须选择的多个选项。 可以管理每组变体的库存，因为它们代表一个具有不同SKU的单独产品。 例如，可配置产品的颜色和大小的组合与目录中的特定SKU相关联。<br/>`virtual`  — 一种不需要装运且不存放在库存中的无形产品。 示例包括服务、成员资格和订阅。<br/>`bundle`  — 一组可定制的简单产品，这些产品一起销售。 |
...
| product_line | 确定产品是否可在商店中销售。 值：<br/>`1`  — （是）产品已启用，并可供销售。<br/>`2`  — （否）该产品已禁用，不可销售。 |
| `tax_class_name` | 与此产品关联的税分类的名称。 |
| `visibility` | 确定产品在目录中是否可见，以及产品是否可用于搜索。 值：<br/>`Not Visible Individually`  — 产品不包括在产品清单中，尽管它可能作为其他产品的变体提供。<br/>`Catalog`  — 产品出现在所有目录列表中。<br/>`Search`  — 该产品可用于搜索操作。<br/>`Catalog, Search`  — 产品包含在目录列表中，也可以进行搜索。 |
| `price` | 产品在您的商店中销售的价格。 |
...
| save_rewrites_history | 在提供值时 `1` 使用新的 `url_key`，则会生成新的301 URL重写，以便将旧URL重定向到新URL。 |
...
| `base_image` | 产品页面上主图像的相对路径。 Commerce以字母顺序的文件夹结构在内部存储文件。 您可以查看导出数据中每个图像的确切位置。 例如： `/sample_data/m/b/mb01-blue-0.jpg`<br/>要上传新图像或覆盖现有图像，请输入文件名，其前面加有正斜杠。 例如： `/image.jpg` |
...
| `map_enabled` | 确定配置中是否启用了最低广告价格。 值：<br/>`1` — （是）启用MAP。<br/>`0` （或空白） — （无）不启用MAP。 |
| `gift_message_available` | 确定产品购买中是否可以包含礼品消息。 值：<br/>`1`  — （是）向客户呈现包含礼品消息的选项。<br/>`0` （或空白） — （否）不向客户显示包含礼品消息的选项。 |
...
| `page_layout` | 确定产品页面的页面布局。 值：<br/>`No layout updates`  — 不更改页面布局。<br/>`1 column`  — 将一列式布局应用于产品页。<br/>`2 columns with left bar`  — 将带有左侧栏的双列布局应用于产品页面。<br/>`2 columns with right bar`  — 将带有右侧栏的双列布局应用于产品页面。<br/>`3 columns`  — 将三列布局应用于产品页。<br/>`empty`  — 将空白布局应用到产品页面。 |
| `product_options_container` | 如果产品有多个选项，则确定它们在产品页面上的显示位置。 值：产品信息列/信息列后的块 |
| `msrp_display_actual_price_type` | 确定客户可在何处看到产品的实际价格。 值：<br/>`In Cart`  — 显示购物车中的实际产品价格。<br/>`Before Order Confirmation`  — 在订单确认之前显示结账流程结束时实际产品价格。<br/>`On Gesture`  — 当客户单击 _单击以获取价格_ 或 _这是什么？_ 链接。 |
...
| `use_config_min_sale_qty` | 确定是否使用最小数量的默认配置设置，它对应于“使用配置设置”复选框的状态。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
| `max_cart_qty` | 指定单笔订单可购买产品的最大数量。 |
| `use_config_max_sale_qty` | 确定是否使用最大数量的默认配置设置，该设置对应于“使用配置设置”复选框的状态。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
| `is_in_stock` | 指示产品是否有库存。 |
| `notify_on_stock_below` | 指定触发 _缺货_ 通知。 |
| `use_config_notify_stock_qty` | 确定默认配置设置是否用于触发库存级别通知，并与使用配置设置复选框的状态相对应。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
| `manage_stock` | 确定是否使用库存控制来管理产品。 值：<br/>`1` — （是）激活全面库存控制以管理产品的库存水平。<br/>`0` （或空白） — （否）系统不跟踪当前库存的物料数量。 |
| `use_config_manage_stock` | 确定是否使用用于管理库的默认配置设置，该设置对应于“使用配置设置”复选框的状态。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
| `use_config_qty_increments` | 确定是否使用数量增量的默认配置设置，该设置对应于“使用配置设置”复选框的状态。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
| `qty_increments` | 确定构成数量增量的产品数量。 |
| `use_config_enable_qty_inc` | 确定是否使用默认配置设置来启用数量增量，该设置对应于“使用配置设置”复选框的状态。 值：<br/>`1` — （是）<br/>`0` （或空白） — （无） |
...
| `crosssell_position` | 确定在中列为交叉销售产品的SKU的位置（排序顺序）。 `crosssell_skus` 列。 |
| `upsell_skus` | 列出已标识为追加销售的每种产品的SKU。 |
| `upsell_position` | 确定SKU在AEM中作为追加销售产品列出的位置（排序顺序）。 `upsell_skus` 列。 |
...
| `_associated_sku` | 与分组产品关联的产品的SKU。 | 分组的产品 <br/>捆绑产品 |
| `_associated_default_qty` | 确定包含的关联产品的数量。 | 可配置产品<br/>分组的产品<br/>捆绑产品 |
| `_associated_position` | 确定关联产品与其他关联产品一起列出时的位置。 | 可配置产品<br/>分组的产品<br/>捆绑产品 |
...
![可配置产品 — 导出的数据](./assets/data-exported-configurable-product.png){width="600" zoomable="yes"}
...
通过“高级价格导入/导出”，您可以快速更新产品组和层价格的定价信息。 目标进程 [导入](data-import.md) 和 [导出](data-export.md) 高级价格数据与任何其他实体类型相同。 示例CSV文件包含每个支持高级定价的产品类型的层价和组价。 更改高级定价不会影响产品记录的其余部分。
...
|--- |--- |
| `sku` | （必需）库存单位是用于跟踪库存的唯一字母数字标识符。 SKU的长度最多可为64个字符。 例如： `sku123`<br/>**_注意：_**超过64个字符的SKU会导致导入失败。 |
| `tier_price_website` | 此 [网站代码](../stores-purchase/stores.md#add-websites) 标识每个提供层定价的网站。 例如： `-  website1 -  All Websites [USD]` |
| `tier_price_customer` | 标识 [客户组](../customers/customer-groups.md) 在提供层定价的情况下。 例如： `-  ALL GROUPS -  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
...
| `tier_price` | 产品的折扣价格。 对象 [捆绑产品](../catalog/product-create-bundle.md)，层级价格按百分比计算。 |
| `group_price_website` | 此 [网站代码](../stores-purchase/stores.md#add-websites) 每个有群组定价的网站的ID。 如果指定多个网站，请用逗号分隔每个网站，不带空格。 例如： `-  website1 -  All Websites [USD]` |
| `group_price_customer_group` | 标识提供组定价的客户组。 例如： `-  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
| `group_price` | 产品的折扣组价。 对象 [捆绑产品](../catalog/product-create-bundle.md)，组价格按百分比计算。 |
