...
| `meta_keywords` | Meta关键字仅对搜索引擎可见，并且被某些搜索引擎忽略。 选择用逗号分隔的高值关键字。 例如： `keyword1`，`keyword2`，`keyword3` |
| `meta_description` | Meta描述提供了用于搜索结果列表的产品简要概述。 理想情况下，元描述长度应介于150到160个字符之间，尽管字段最多可接受255个字符。 |
