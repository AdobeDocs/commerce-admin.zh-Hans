...
此 `composer.json` 文件声明文件包列表，而 `composer.lock` 文件存储用于构建Adobe Commerce B2B安装的软件包（每个软件包及其依赖项的完整版本）的完整列表。

以下参考文档是从生成的 `composer.lock` ，它涵盖Adobe Commerce B2B 1.4.2中包含的必需包。
