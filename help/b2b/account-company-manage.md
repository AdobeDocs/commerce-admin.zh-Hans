...
此 _[!UICONTROL Companies]_页面列出所有当前公司帐户，不考虑状态。 任何待处理的审批请求都将显示在列表顶部。 标准 [工作区控件](../getting-started/admin-workspace.md) 可用于筛选列表，更改 [列布局](../getting-started/admin-grid-controls.md)、保存视图或导出数据。

此 _[!UICONTROL Actions]_网格上方的控件可用于将操作应用于多个公司记录。 例如，您可以选择多个请求，并在单个操作中激活帐户，而不是批准每个公司请求。 可用的操作取决于 [权限](../systems/permissions.md) 用于分配给管理员用户帐户的角色。

使用 _[!UICONTROL Search]_函数以查找以下位置中的公司：**公司**按关键字网格。 搜索索引关键字来自&#x200B;**公司名称**和&#x200B;**父级**列。 您可以按以下项过滤&#x200B;**公司类型**仅显示单个公司、仅显示母公司或仅显示子公司。

![公司网格](./assets/companies-grid-view.png){width="700" zoomable="yes"}

## 公司角色资源

此 [角色资源](../systems/permissions-user-roles.md#role-resources) 设置决定了以下能力：
...
必须为设置这些角色资源 [用户角色](../systems/permissions-user-roles.md) 指定给管理员用户帐户的密码。
...
1. 从 **[!UICONTROL Actions]** 控件，选择 **[!UICONTROL Set Active]**.
...
1. 在 _管理员_ 侧栏，转到 [!UICONTROL **客户**] > [!UICONTROL **所有客户**].

1. 从 **[!UICONTROL Actions]** 菜单，选择以下选项之一：
...
1. 在 _操作_ 最右侧的列，选择 [!UICONTROL **编辑**].

1. 选择 [!UICONTROL **帐户信息**] 选项卡。

1. 设置 [!UICONTROL **客户活动**] 到 `Yes` 或 `No`.
...
1. 从 **[!UICONTROL Actions]** 控件，选择 **[!UICONTROL Block]**.

1. 提示确认时，单击 **[!UICONTROL OK]**.

### 删除公司帐户

无法恢复已删除的公司帐户。 与公司关联的用户帐户的状态设置为 `Inactive` 并从用户帐户的配置文件中删除公司ID。 有关公司活动和交易的信息保留在系统中。

1. 从 **[!UICONTROL Actions]** 控件，选择 **[!UICONTROL Delete]**.
...
1. 从 **[!UICONTROL Actions]** 控件，选择 **[!UICONTROL Convert Currency]**.

1. 提示确认时，单击 **[!UICONTROL OK]**.

1. 选择 **[!UICONTROL Credit Currency]** 用于所选公司帐户。
...
1. 从 **[!UICONTROL Actions]** 控件，选择 **[!UICONTROL Edit]**.

   每个可更新的值都会显示在文本框中。

   ![公司帐户的快速编辑](./assets/companies-grid-quick-edit.png){width="700" zoomable="yes"}
...
1. 选择 **[!UICONTROL Edit]** 从 _[!UICONTROL Action]_列。
...
销售代表是 [管理员用户](../systems/permissions.md) 被分配为公司帐户的联系人并接收所有自动发送的 [电子邮件消息](../b2b/enable-basic-features.md#configure-company-email-options) 系本公司关连人士。 每个公司帐户只能分配一个销售代表，但单个销售代表可以管理多个公司帐户。 除非分配了其他管理员用户，否则默认管理员用户帐户将被分配为销售代表。
...
1. 设置 **[!UICONTROL Sales Representative]** 分配给要指定为公司联系点的管理员用户。
...
创建公司后，具有适当权限的管理员用户可以使用 [!UICONTROL Company Hierarchy] 部分，通过编辑指定的母公司和分配相关公司来构建母公司组织。

如果已将公司添加到层次结构，则 [!UICONTROL Company Hierarchy] 网格显示网格中的父公司和所有分配的公司。

请参阅 [管理公司层次结构](assign-companies.md) 以了解更多信息。
...
| [!UICONTROL Delete] | 删除所选的公司帐户。 与已删除的公司关联的用户帐户状态设置为 `Inactive` 并从用户帐户的配置文件中删除公司ID。 有关公司活动和交易的信息保留在系统中。 |
...
| [!UICONTROL Company Type] | 类型 [公司](manage-companies.md). 选项： <br/>**[!UICONTROL Company]**— 默认情况下，新公司是作为单个公司创建的。<br/>**[!UICONTROL Parent]**  — 本公司为其他公司的母公司。 <br/>**[!UICONTROL Child]**— 此公司与母公司相关。 |
...
| [!UICONTROL Group/Shared Catalog] | 列名称取决于配置中是否启用了共享目录。 选项： <br/>**[!UICONTROL Customer Group]**— 如果配置中未启用共享目录，请指定 [客户组](../customers/customer-groups.md) 公司所属的。<br/>**[!UICONTROL Shared Catalog]**  — 如果在配置中启用了共享目录，则指定分配给客户的共享目录的名称。 |
...
通过更改 [列布局](../getting-started/admin-grid-controls.md) 网格的URL。
...
| [!UICONTROL VAT/TAX ID] | 此 [增值税](../stores-purchase/vat.md) 某些管辖区为报税目的而分配给公司的编号。 要配置要显示在店面中的客户VAT/TAX ID，请参阅 [创建新帐户选项](../configuration-reference/customers/customer-configuration.md). |
| [!UICONTROL Credit Limit] | 扩展到公司帐户的信用额度。 |
| [!UICONTROL Credit Currency] | 商店接受以公司贷记进行购买的货币。 |
| [!UICONTROL Status] | 指示 [状态](account-company-approve.md) 公司帐户的ID。 选项： <br/>**[!UICONTROL Active]**— 公司帐户由商店管理员批准。 公司管理员和关联成员可以从店面登录帐户并进行购买。<br/>**[!UICONTROL Pending Approval]**  — 已提交打开公司帐户的请求，但尚未获得商店管理员的批准。 <br/>**[!UICONTROL Rejected]**— 已提交打开公司帐户的请求，但未获得商店管理员的批准。 用于提交请求的初始登录凭据被阻止。<br/>**[!UICONTROL Blocked]**  — 公司成员可以登录并访问目录，但不能进行购买。 商店管理员可能会阻止信誉不佳的公司帐户。 商店管理员可随时删除帐户上的块。 |
...
| [!UICONTROL Login as Customer] | 允许管理员用户 [以客户身份登录店面](../customers/login-as-customer.md) 帮他们处理命令。 |
| [!DNL Delete Company] | 删除公司帐户。 与公司关联的用户帐户的状态设置为 `Inactive` 并从用户帐户的配置文件中删除公司ID。 有关公司活动和交易的信息保留在系统中。 |
...
|---------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Company Legal Name] | 公司的完整法定名称。 |
| [!UICONTROL VAT / TAX ID] | 税金或 [增值税](../stores-purchase/vat.md) 为纳税申报目的而分配给公司的编号。 |
| [!UICONTROL Reseller ID] | 为纳税申报目的而分配给公司的转售编号。 |
...
| 字段 | 描述 |
|-----------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Customer Group] | 指示 [客户组](../customers/customer-groups.md) 或 [共享目录](catalog-shared.md) 分配给公司的属性。 |
