...
Adobe Commerce或Magento Open Source的单次安装可以有多个共享管理员的商店。 位于同一网站下的商店具有相同的IP地址和域，使用相同的安全证书，并共享单个结账过程。
...
Adobe Commerce或Magento Open Source的多站点安装必须从管理员以及服务器的命令行中进行配置。 Commerce [配置指南](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/multi-sites/ms-overview.html)提供了有关配置服务器环境的详细说明。
