...
1. 如果您使用DHL REST API，请将&#x200B;**[!UICONTROL DHL Type]**&#x200B;设置为`DHL REST`。

   如果您使用的是DHL XML API，请将&#x200B;**[!UICONTROL DHL Type]**&#x200B;设置为`DHL XML`。

   >[!NOTE]
   >
   >DHL REST API是与DHL集成的首选方法。 XML API已弃用，并且可能在未来版本中删除。

1. 使用DHL提供的凭据完成以下字段：

如果您使用的是DHL REST API，则必须提供以下凭据：

    - **[!UICONTROL API KEY]**
    - **[!UICONTROL API SECRET]**

如果您使用的是DHL XML API，则必须提供以下凭据：
...
1. 根据您的要求配置手续费选项。
   ...
   - 对于&#x200B;**[!UICONTROL Handling Fee]**，根据您选择用于计算金额的方法，输入要收取的金额。
     ...
     例如，如果费用基于固定费用，则以小数形式输入金额，如`4.90`。 但是，如果手续费基于订单的百分比，则按百分比输入金额。 例如，如果您对订单的6%收费，则输入值为`6`。
...
1. DHL提供了&#x200B;**[!UICONTROL sandbox mode]**&#x200B;选项。 如果您使用的是沙盒模式，请将&#x200B;**[!UICONTROL sandbox mode]**&#x200B;设置为`Yes`。
如果您使用的是实时模式，请将**[!UICONTROL sandbox mode]**&#x200B;设置为`No`。

   >[!NOTE]
   >
   >沙盒模式仅用于测试目的。 它允许您测试与DHL的集成，而不会影响实时存储。
