...
如果商家或客户位于同一国家/地区，则可将商业配置为根据商家或客户地址征收增值税。 增值税的计算通常基于发运目的地，而不是其原始点。 在大多数情况下，根据客户送货地址计算增值税的配置设置便已足够。
...
- 出售 [可下载的产品](../catalog/product-create-downloadable.md)，或 _数字商品_，增值税率基于配送目的地，而不是商家所在地。 请参阅 [数字商品的供应地点](taxes.md#place-of-supply-for-digital-goods-eu).

>[!TIP]
>
>一些跨境和B2B运输的税务要求更为复杂。 要扩展Commerce安装的本机功能，请考虑从以下位置添加税务管理解决方案： [市场](https://marketplace.magento.com/extensions/accounting-finance/taxes.html).
...
在某些企业到企业的交易中，不评定增值税。 商业可以验证客户的VAT ID，以确保正确评估（或未评估）VAT。 请参阅 [VAT ID验证](#vat-id-validation).
...
1. 在 _管理员_ 侧栏，转到 **[!UICONTROL Stores]** > _[!UICONTROL Taxes]_>**[!UICONTROL Tax Zones and Rates]**.
   ...
   ![设置客户税分类](./assets/vat-zones.png){width="600" zoomable="yes"}

1. 确保有一个适合用于VAT的客户税分类。

   对于此实例，请确保存在名为的客户税分类 _零售客户_. 如果此税分类不存在，请单击 **[!UICONTROL Add New Tax Rate]**.

1. 输入 **[!UICONTROL Tax Identifier]** 新税种。

   所有税率均显示在 _税率_ 中的字段 _税则信息_ 在创建税则时。

1. 要设置邮政编码范围（从/到），请选择 **[!UICONTROL Zip/Post is Range]** 复选框。

1. 选择 **[!UICONTROL Country]** 适用税率的地方。

1. 输入 **[!UICONTROL Rate Percent]** 用于计算购买时的税率。
...
1. 展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL Additional Settings]** 部分。
   ...
   ![设置产品税分类](./assets/tax-class-additional-settings.png){width="600" zoomable="yes"}

1. 下 _产品税分类_，单击 **[!UICONTROL Add New Tax Class]**.

1. 要将新分类添加至可用产品税分类列表并创建三个新分类，请输入 **[!UICONTROL Name]** ，然后单击复选标记：

   - `VAT Standard`
   - `VAT Reduced`
   - `VAT Zero`

1. 单击 **[!UICONTROL Save Class]** 添加的每个新类。
...
1. 单击 **[!UICONTROL Save Rule]** 每个费率。

### 步骤5：将税分类应用于产品

1. 在 _管理员_ 侧栏，转到 **[!UICONTROL Catalog]** > **[!UICONTROL Manage Products]**.

1. 在编辑模式下从目录打开产品。

1. 在 _常规_ 页面，查找 **[!UICONTROL Tax Class]** 选项，然后选择 **[!UICONTROL VAT Class]** 适用于产品的属性。

1. 完成后，单击 **[!UICONTROL Save]**.

   ![将税分类应用于产品](./assets/vat-apply-classes.png){width="600" zoomable="yes"}

## 字段描述

### 存储信息

Commerce使用以下功能 [存储信息配置设置](../configuration-reference/general/general.md#store-information) 以根据商家信息计算增值税。

**[!UICONTROL VAT Number]**  — 分配给商家的增值税号。

**[!UICONTROL Validate VAT Number]** - [增值税验证](#vat-id-validation) 确认增值税编号与 [欧洲委员会](https://ec.europa.eu/taxation_customs/vies/) 数据库。

### 客户信息

Commerce使用以下字段根据以下字段计算增值税 [客户信息](../customers/account-dashboard-account-information.md))。

#### 帐户信息

**[!UICONTROL Tax/VAT Number]**  — 如果适用，是指分配给客户的税号或增值税号。

#### 地址

**[!UICONTROL VAT Number]**  — 如果适用，与客户的特定帐单或送货地址关联的增值税编号。 出售 [数字商品](taxes.md#place-of-supply-for-digital-goods-eu))在欧盟内，增值税的金额基于配送目的地。

### 客户帐户

Commerce使用以下功能 [客户配置设置](../customers/account-options-new.md) 以计算VAT。

**[!UICONTROL Show VAT Number on Storefront]**  — 确定“客户增值税编号”字段是否包含在客户帐户中可用的通讯簿中。

**[!UICONTROL Default Value for Disable Automatic Group Changes Based on VAT ID]** - VAT ID是用于增值税验证的客户增值税编号的内部标识符。 在增值税验证期间，商务会确认该数字与 [欧洲委员会](https://ec.europa.eu/taxation_customs/vies/) 数据库。 客户可以根据验证结果自动分配到四个默认客户组之一。

## VAT ID验证

_VAT ID验证_ 根据商户和客户的区域设置，自动计算在欧盟(EU)内发生的B2B交易所需的税费。 Commerce使用 [欧洲委员会][1] 服务器。
...
如果启用，则可以通过选中 _[!UICONTROL Customer Information]_页面。
...
如果客户使用Google Express Checkout、PayPal Express Checkout或其他外部签出方法，则会在外部支付网关的侧面完全执行签出。 对于此方案， _验证每个交易记录_ 无法应用设置，且客户组在结账期间无法更改。
...
| 税则#1 |  |
|--- |--- |
| 客户税分类 | 客户税分类必须包括： <br />国内客户之类别。 <br />VAT ID格式不正确的客户的类。<br />VAT ID验证失败的客户的类。 |
...
| 税则#3 | （虚拟和可下载产品需要） |
|--- |--- |
| 客户税分类 | 客户税分类必须包括： <br/>A国内客户类别 <br/>VAT ID无效客户的分类客户的VAT ID验证失败的客户的A分类 |
| 产品税分类 | 虚拟产品的类。 |
| 税率 | 贸易商所在国家/地区的增值税率。 |
...
创建 [税则](tax-rules.md) 以有效使用VAT ID验证。

- 税则包括税率和 [税种](tax-class.md).
- 税分类分配至 [客户组](../customers/customer-groups.md).

#### 步骤3：启用并配置VAT ID验证

1. 在 _管理员_ 侧栏，转到 **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. 如有必要，请设置 **[!UICONTROL Store View]** 用于配置。

1. 在左侧面板中，展开 **[!UICONTROL Customers]** 并选择 **[!UICONTROL Customer Configuration]**.

1. 展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL Create New Account Options]** 部分。
...
1. 设置 **[!UICONTROL Enable Automatic Assignment to Customer Group]** 到 `Yes` 并根据需要完成以下字段。
...
1. 在中继续 _常规_ 配置页面，展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL Countries Options]** 部分。

   ![国家/地区选项](../configuration-reference/general/assets/general-country-options.png){width="600" zoomable="yes"}

1. 在 **[!UICONTROL European Union Countries]** 列表，验证是否选择了欧盟的每个成员国。
