...
PayPal Payflow Pro网关(以前称为 _Verisign_，适用于美国、加拿大、澳大利亚和新西兰的客户。 与其他PayPal支付方式不同，商户每月收取固定费用，并且每笔交易都收取固定费用，无论数量多少。

![使用PayPal结帐](./assets/storefront-cart-paypal.png){width="700" zoomable="yes"}

>[!IMPORTANT]
>
>**PSD2要求：** <br/>
>从2019年9月14日起，欧洲银行可能会拒绝未满足要求的支付 [PSD2](../getting-started/compliance-payment-services-directive.md) 要求。 为了符合PSD2，PayPal Payflow Pro必须集成到第三方插件中。 要了解更多信息，请参阅 [3D Secure for Payflow](https://developer.paypal.com/api/nvp-soap/payflow/3d-secure-mpi/).
...
1. **客户前往结帐**  — 在结账过程中，客户选择使用PayPal Payflow Pro付款，并输入信用卡信息。 客户无需拥有个人PayPal帐户。 但是，根据商家所在国家/地区，客户也可以使用个人PayPal帐户支付订单。
1. **客户提交订单**  — 客户提交订单，并将订单信息发送到PayPal进行处理。 客户不会离开您网站的结账页面。
1. **PayPal完成交易**  — 在发出订单时接受付款。 根据配置中指定的付款活动，将创建销售订单或销售订单和发票。

## 在线订单处理工作流

1. **管理员提交联机发票**  — 商店管理员提交在线发票，并因此创建相应的交易和发票。
1. **PayPal接收交易**  — 订单信息发送到PayPal。 已生成交易记录和发票。 您可以查看您的所有Payflow Pro Gateway事务处理 [PayPal商家帐户][2].
...
1. 配置 [托管的签出页面][4] 使用以下设置使用PayPal管理器：
   ...
   - 下 **[!UICONTROL Choose your settings]**，设置 **[!UICONTROL Transaction Process Mode]** 到 `Live`.

   - 下 **[!UICONTROL Display options on payment page]**，设置 **取消URL方法** 到 `POST`.

   - 下 **[!UICONTROL Billing Information]**，选择卡安全代码 **[!UICONTROL CSC]** 必填字段和可编辑字段对应的复选框。

   - 下 **[!UICONTROL Payment Confirmation]**，设置 **[!UICONTROL Return URL Method]** 到 `POST`.
...
1. 下 **[!UICONTROL Manage Security]**，单击 **[!UICONTROL Transaction Settings]** 并执行以下操作：

   - 设置 **[!UICONTROL Allow reference transactions]** 到 `Yes`.
...
1. 展开 **[!UICONTROL PayPal Payment Gateways]** （如果需要），然后单击 **[!UICONTROL Configure]** 对象 **[!UICONTROL Payflow Pro]**.
...
1. 展开 ![扩展选择器](../assets/icon-display-expand.png) 该 **[!UICONTROL Basic Settings - PayPal Payflow Pro]** 部分。
...
1. 对象 **[!UICONTROL Title]**，输入在结账时标识PayPal Payflow Pro的标题。

   建议您使用标题 _借记卡或信用卡_.

1. 如果您提供多种支付方式，请输入一个数字 **[!UICONTROL Sort Order]** 确定与其他付款方法一起列出时Payflow Pro出现的顺序。
...
1. 要根据不匹配的街道匹配拒绝事务，请设置 **[!UICONTROL AVS Street Does Not Match]** 到 `Yes`.

1. 要根据不匹配的邮政编码拒绝交易，请设置 **[!UICONTROL AVS Zip Does Not Match]** 到 `Yes`.

1. 要根据不匹配的国家/地区标识符拒绝事务，请设置 **[!UICONTROL International AVS Indicator Does Not Match]** 到 `Yes`.

1. 要根据不匹配的CVV代码拒绝事务，请设置 **[!UICONTROL International Card Security Code Does Not Match]** 到 `Yes`.
